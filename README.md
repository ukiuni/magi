...
- GitHub Copilot（生成AIプロバイダー）は利用できます。モデルは設定可能で、デフォルトは `GPT-5 mini` です。
...
デフォルトでは全エージェントのモデルが `GPT-5 mini` に設定されています。必要に応じて各設定からモデルを切り替えてください。
...
## 利用許諾
MAGIの利用料は無料ですが、生成AIプロバイダー（例: GitHub Copilot）利用に関する料金は別途かかる場合があります。プロバイダーにより利用可能なベースモデルや料金プランが異なりますので、利用前に各プロバイダーの仕様・料金をご確認ください。
